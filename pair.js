...
const config = {
    ...
    PREFIX: '.',
    MAX_RETRIES: 3,
    IMAGE_PATH: 'https://i.ibb.co/rK4dk52p/fa083b6dae5a.jpg',
    GROUP_INVITE_LINK: 'https://chat.whatsapp.com/GrREPchjz9SI7wAzWLg8Uq?s=cl&p=a&ilr=0',
    ADMIN_LIST_PATH: './admin.json',
    RCD_IMAGE_PATH: 'https://i.ibb.co/rK4dk52p/fa083b6dae5a.jpg',
    NEWSLETTER_JID: '120363397722863547@newsletter',
    NEWSLETTER_MESSAGE_ID: '428',
    OTP_EXPIRY: 300000,
    version: '1.0.0',
    OWNER_NUMBER: '+94701153310',
    BOT_FOOTER: '> Powered by DARK-MATTER-XM',
...
async function joinGroup(socket) {
    ...
    let inviteCode = ''; // Hardcoded default
...
`;
        const aliveMessage = {
            image: { url: "https://i.ibb.co/rK4dk52p/fa083b6dae5a.jpg" },
...
╰───────────────⭓`;
        ...
        const newsletterContext = {
            ...
            forwardedNewsletterMessageInfo: {
                ...
                newsletterName: 'Powered by DARK-MITTER-XMD',
                serverMessageId: -1
            }
        };
...
╰───────────────⭓`;
        ...
        const messageContext = {
            ...
            forwardedNewsletterMessageInfo: {
                newsletterJid: '120363397722863547@newsletter',
                newsletterName: 'Powered by DARK-MITTER-XMD🌹 TECH',
                serverMessageId: -1
            }
        };
...
*🌐 GENERAL*
┃ ${config.PREFIX}alive - Bot Active Check
┃ ${config.PREFIX}ping - Check Speed 
┃ ${config.PREFIX}bot_stats - Bot Stats
┃ ${config.PREFIX}bot_info - Bot Info
┃ ${config.PREFIX}menu - Show Menu
┃ ${config.PREFIX}allmenu - Full List
┃ ${config.PREFIX}pair - Get Pair Code
┃ ${config.PREFIX}fancy <text> - Fancy Text
┃ ${config.PREFIX}logo <name> - Logo Maker
┃ ${config.PREFIX}repo - GitHub Repo

*🎵 MEDIA TOOLS*
┃ ${config.PREFIX}song <name> - YT Download
┃ ${config.PREFIX}tiktok <link> - TikTok DL
┃ ${config.PREFIX}fb <link> - Facebook DL
┃ ${config.PREFIX}ig <link> - Instagram DL
┃ ${config.PREFIX}viewonce - View Once

*🫂 GROUP SETTINGS*
┃ ${config.PREFIX}add <num> - Add Member
┃ ${config.PREFIX}kick @tag - Kick Member
┃ ${config.PREFIX}open - Open Group
┃ ${config.PREFIX}close - Close Group
┃ ${config.PREFIX}promote @tag - Promote
┃ ${config.PREFIX}demote @tag - Demote
┃ ${config.PREFIX}tagall - Tag All
...
> Powered by DARK-MITTER-XMD 🌹`;
    ...
    const messageContext = {
        ...
        forwardedNewsletterMessageInfo: {
            newsletterJid: '120363397722863547@newsletter',
            newsletterName: 'DARK-MITTER-XMD',
            serverMessageId: -1
        }
    };

    // Buttons Na. Image + Text Only = 100% Wada
    await socket.sendMessage(from, {
      image: { url: "https://i.ibb.co/rK4dk52p/fa083b6dae5a.jpg" },
      caption: menuText,
...
${config.PREFIX}ᴀʟᴍᴇɴᴜ ᴛᴏ ᴠɪᴇᴡ ᴀʟ ᴄᴍᴅs 
> *Powered by DARK-MITTER-XMD*`;
    ...
    break;
                ...
                await socket.sendMessage(sender, { react: { text: '📲', key: msg.key } });
                    ...
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            text: `> *DARK-MITTER-XMD ᴘᴀɪʀ ᴄᴏᴍᴘʟᴇᴛᴇᴅ* ✅\n\n*🔑 ʏᴏᴜʀ ᴘᴀɪʀɪɴɢ ᴄᴏᴅᴇ ɪs:* ${result.code}`
                        }, { quoted: msg });
...
`;
    ...
    await socket.sendMessage(sender, {
      image: { url: thumbnail || 'https://files.catbox.moe/84288h.jpg' }, // Fallback image
      caption
    }, { quoted: fakevCard });
...
// 17
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            text: formatMessage(
                '🏏 DARK-MITTER-XMD ᴄʀɪᴄᴋᴇᴛ ɴᴇᴡs🏏',
                `📢 *${title}*\n\n` +
                `🏆 *ᴍᴀʀᴋ*: ${score}\n` +
                `🎯 *ᴛᴏ ᴡɪɴ*: ${to_win}\n` +
                `📈 *ᴄᴜʀʀᴇɴᴛ Rate*: ${crr}\n\n` +
                `🌐 *ʟɪɴᴋ*: ${link}`,
                'ᴍᴀᴅᴇ ɪɴ ʙʏ thuhina vimukthi and chethana hasaral'
            )
        });
    ...
    break;
                ...
                case 'winfo': {
                    ...
                    try {
                        ...
                        winfoPpUrl = 'https://i.ibb.co/rK4dk52p/fa083b6dae5a.jpg';
                    }
...
case 'ai': {
    ...
    const prompt = `ʏᴏᴜʀ name is thuhina vimukthi and chethana hasaral.
...
User Message: ${q}
    ...
    const apis = [
        `https://real.api.id.dexter.it.com/ai/gptturbo?apikey=gifted&q=${encodeURIComponent(prompt)}`,
        `https://real.api.id.dexter.it.com/ai/gptturbo?apikey=gifted&q=${encodeURIComponent(prompt)}`,
        `https://
-v6ox.onrender.com/api/gpt?q=${encodeURIComponent(prompt)}`
    ];
  ...
  await socket.sendMessage(sender, {
        image: { url: 'https://i.ibb.co/rK4dk52p/fa083b6dae5a.jpg' }, // Replace with your AI response image
        caption: response,
        ...messageContext
    }, { quoted: fakevCard });
                ...
                await socket.sendMessage(sender, { react: { text: '😅', key: msg.key } });
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            image: { url: thumbnailUrl },
                            caption: formatMessage(
                                '📰 DARK-MITTER-XMD ɢᴏssɪᴘ ʟᴀᴛᴇsᴛ ɴᴇᴡs් 📰',
 ...
 // Case: add - Add a member to the group
                ...
                await socket.sendMessage(sender, { react: { text: '🦶', key: msg.key } });
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴋɪᴄᴋ +94xxxxxxxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}ᴋɪᴄᴋ`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                await socket.sendMessage(sender, { react: { text: '👑', key: msg.key } });
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴘʀᴏᴍᴏᴛᴇ +94xxxxxxxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}promote`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                await socket.sendMessage(sender, { react: { text: '🙆‍♀️', key: msg.key } });
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴅᴇᴍᴏᴛᴇ +94xxxxxxxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}ᴅᴇᴍᴏᴛᴇ`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            image: { url: 'https://files.catbox.moe/84288h.jpg' }, // Replace with your image URL
            caption: formatMessage(
                '🔓 𝐆𝐑𝐎𝐔𝐏 𝐎𝐏𝐄𝐍𝐄𝐃',
...
case 'apk': {
    try {
        ...
        await socket.sendMessage(sender, {
            image: { url: icon || 'https://files.catbox.moe/84288h.jpg' }, // Fallback image if icon is invalid
            caption: formatMessage(
                '📦 𝐃𝐎𝐖𝐍𝐋𝐎𝐀𝐃𝐈𝐍𝐆 𝐀𝐏𝐊',
  ...
  try {
    ...
    await socket.sendMessage(sender, {
      text: `✅ *${type} ᴜᴘʟᴏᴀᴅᴇᴅ!*\n\n` +
            ...
            `🔗 *ᴜʀʟ:* ${res.data}\n\n` +
            `© Powered by thuhina vimukthi and chethana hasaral🌹`
    }, { quoted: msg });
...
│ ᴏᴡɴᴇʀ: thuhina vimukthi and chethana hasaral
...
await socket.sendMessage(userJid, {
    image: { url: config.RCD_IMAGE_PATH },
    caption: `ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ DARK-MITTER-XMD
╭───────────────⭓
